...
function Send-AzServiceBusMessage {
    ...
    $message = [PSCustomObject] @{ "Body" = "new blob is added"; }
...
if (!$serviceBussInstalled) {
    #Install-Module Az.ServiceBus -Scope CurrentUser
}

Send-AzServiceBusMessage -ResourceGroupName India_rg2 `
    -NamespaceName trrreeeee `
    -QueueName test `
    -PolicyName RootManageSharedAccessKey
